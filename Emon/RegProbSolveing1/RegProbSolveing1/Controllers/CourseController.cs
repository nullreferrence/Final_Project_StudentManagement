...
namespace RegProbSolveing1.Controllers
{
    public class CourseController : Controller
    {
        ...
        public ActionResult CourseStatus(int? departmentId)
        {
            ViewBag.DepartmentId = new SelectList(db.DepartmentDbSet, "DepartmentID", "DeptCode");
            if (departmentId != null)
            {
                return
                    // View(db.CourseDbSet.Include(s => s.Department).Where(s => s.Department.DepartmentID == departmentId));
                    View(db.CourseDbSet.Include(s => s.Department).Where(s => s.Department.DepartmentID == departmentId));
            }
            else
            {
                return View(db.CourseDbSet.Include(s => s.Department));
            }
        ...
        public PartialViewResult FilteredCourse(int? depId)
        {
            if (depId != null)
            {
                var model = db.CourseDbSet.Include(s => s.Department).Where(s => s.Department.DepartmentID == depId);
                return PartialView("~/Views/Shared/_CoursePartial.cshtml", model);
            }
            else
            {
                return PartialView("~/Views/Shared/_CoursePartial.cshtml", db.CourseDbSet.Include(s => s.Department));
            }
        }
